...
/* movedriver.prx — a tiny kernel-mode helper (see movedriver/movedriver.c)
 * that runs sceIoRename with briefly-elevated privilege, following the
 * same pattern real PSP tools like CMFileManager PSP use for privileged
 * IO. Confirmed on real hardware that plain sceIoRename fails for any
 * cross-directory move on this CFW (0x80010011), which is exactly what
 * sorting a game into a CAT_xx folder needs — this fixes that instantly
 * instead of falling back to a slow copy+delete.
 *
 * pspIoRename() below is declared via movedriver-stub.S, generated by
 * movedriver's own Makefile (psp-build-exports -s on the same
 * exports.exp that defines what movedriver.prx exports) — the normal
 * PSPSDK stub-linking mechanism. Calling it is a plain function call;
 * the stub resolves to whatever module currently has that export
 * loaded, which is why the module must actually be loaded and started
 * first, or the call will fail (not crash — sceKernelStartModule etc.
 * simply return an error if the target module isn't running). */
